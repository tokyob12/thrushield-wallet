...
const backgroundReady = Promise.all([
  ...
  console.error("Failed to initialize wallet background state", error);
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.source === CONTENT_SCRIPT_SOURCE && message?.kind === "dapp") {
    ...
    void backgroundReady.then(() => handleDappRequest(message.payload as DappRequestMessage, tabId)).then((response) => {
      if (response) {
  ...
  if (message?.source === "thruShield-internal") {
    backgroundReady
      .then(() => handleInternalMessage(message.payload as InternalMessage))
      .then(sendResponse)
      .catch((error) => {
        sendResponse({
          ok: false,
