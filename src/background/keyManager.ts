...
import { keys, Pubkey, Transaction } from "@thru/sdk";
...
import {
  AUTO_LOCK_EXPIRY_KEY,
  cancelAutoLockTimer,
  resetAutoLockTimer,
  restoreAutoLockTimer,
} from "./autoLock";
...
const UNLOCKED_SESSION_KEY = "thruShield:unlockedSession";
...
export async function lockWallet(): Promise<void> {
  ...
  await cancelAutoLockTimer();
}

async function persistUnlockedSession(): Promise<void> {
  if (!unlockedVault) return;
  await chrome.storage.session.set({
    [UNLOCKED_SESSION_KEY]: { vault: unlockedVault } satisfies UnlockedSession,
  });
}

/** Restore an unlocked wallet after Chrome suspends/restarts the MV3 worker. */
export async function restoreUnlockedSession(): Promise<void> {
  await chrome.storage.session.setAccessLevel({ accessLevel: "TRUSTED_CONTEXTS" });
  const stored = await chrome.storage.session.get([
    UNLOCKED_SESSION_KEY,
    AUTO_LOCK_EXPIRY_KEY,
  ]);
  const session = stored[UNLOCKED_SESSION_KEY] as UnlockedSession | undefined;
  const expiresAt = stored[AUTO_LOCK_EXPIRY_KEY] as number | undefined;

  if (!session?.vault || !expiresAt || expiresAt <= Date.now()) {
    await lockWallet();
    return;
  }

  const vault = normalizeVaultPlaintext(session.vault);
  const activeId =
    vault.accounts.find((account) => account.id === vault.activeAccountId)?.id ??
    vault.accounts[0]?.id;
  if (!activeId) {
    await lockWallet();
    return;
  ...
  await activateAccount(vault, activeId);
  await restoreAutoLockTimer(expiresAt);
}
